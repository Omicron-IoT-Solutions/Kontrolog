...
Once installed, download the Firmware binary file and connect your Kontrolog Max to your computer using the USB-C cable. Then, run the downloaded application and set, in the download tool mode, ESP32 as the chip type and the working mode as development. 
...
Next, download the [.HMI file (verificar y corregir link)](https://github.com/Omicron-IoT-Solutions/Kontrolog-Max/tree/No-Code/Display), which is the code we provide so that you can simulate the HMI screen in NEXTION and which is also the code that is set in the HMI screens that allow you to program the Kontrolog Max locally in No-Code mode.
